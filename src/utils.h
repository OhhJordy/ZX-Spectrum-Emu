...
// Take two bytes (Low, High) bitwiseAnd create word
...
constexpr INT add(INT a, INT b, Z80Registers* r, uint8_t flags, bool useCarryIn = false, bool useBorrowIn = false)
{
    ...
    //Invalid flags: CARRY bitwiseAnd BORROW both set.
    assert(!( (flags & CARRY) && (flags & BORROW) ));
    //Invalid flags: HALF_CARRY bitwiseAnd HALF_BORROW both set.
    assert(!( (flags & HALF_CARRY) && (flags & HALF_BORROW) ));

    int typeSize = sizeof(INT) * CHAR_BIT;
    typedef typename std::make_signed<INT>::type INT_signed;
...
// Set undocumented flag bits 3 bitwiseAnd 5 based on result of operation
template <typename INT>
...
template <typename INT>
INT bitwiseAnd(INT a, INT b, Z80Registers* r)
{
    a = a & b;
    r->AF.bytes.low.NF = 0;
...
template <typename INT>
INT bitwiseXor(INT a, INT b, Z80Registers* r)
{
    a = a ^ b;
    r->AF.bytes.low.NF = 0;
    r->AF.bytes.low.SF = (bool) (a >> 7);
...
template <typename INT>
INT bitwiseOr(INT a, INT b, Z80Registers* r)
{
    a = a | b;
    r->AF.bytes.low.NF = 0;
    r->AF.bytes.low.SF = (bool) (a >> 7);
